...
function git_prompt_config()
{
  ...
  local Lightred="\[\033[1;31m\]" #geand added
  local Lightblue="\[\033[1;34m\]" #geand added
  ...
  local Time12a="[\$(date +%H:%M)]" #geand added
  #local Time12a="\$(date +%H:%M)"
  # local Time12a="(\$(date +%H:%M:%S))"
  # local Time12a="(\@))"
  local PathShort="\w"
  local User="\u@\h:" #geand added
  local Directory="\W"
  if [ "x${GIT_PROMPT_START}" == "x" ]; then
    PROMPT_START="${Yellow}${Time12a}${PathShort}${ResetColor}" #geand added
  ...
  if [ "x${GIT_PROMPT_END}" == "x" ]; then
    PROMPT_END=" \n${Lightblue}${User}${Lightred}${Directory}${ResetColor} $ " #geand added
  ...
  EMPTY_PROMPT="$($prompt_callback)""${Lightblue}${User}${Lightred}${Directory}${ResetColor} $ "
